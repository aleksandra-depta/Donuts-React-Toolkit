...
const Payment = () => {
  ...
  const { street, streetNumber, city, state, zip, name, lastName, phoneNumber, email } = useSelector((store) => store.form);
  const { pickUpId } = useSelector((store) => store.location);
  const { data: location, isLoading, isSuccess, isError, error} = useGetLocationQuery(pickUpId);
      ...
      <Content>
        ...
        <RowContent>
          <Row>
            ...
            <HeadingH4>{ name } { lastName }</HeadingH4> 
          </Row>
          <Row>
      ...
      <Content>
        ...
        <RowContent>
          ...
          <Row>
            { type === 'pick up' ? 
              <HeadingH4>
                { location.data.data.streetNumber } {  location.data.data.street } St, {  location.data.data.city }, {  location.data.data.state } {  location.data.data.zip } 
              </HeadingH4>
            : 
              <HeadingH4>
